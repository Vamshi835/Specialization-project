...
log_path = 'E:\Projects\EBT\logs.log'
print(log_path)
...
ALLOWED_EXTENSIONS = set(['json','png','jpg','jpeg','gif'])
QP_TEMPLATE_SCHEMA = {
...
# ____________________________________________________________ Important Mehod ________________________________________________________________________
...
def test():
    ...
    email="mandalivamshikrishna@gmail.com"
    password="Vamshi835"
    ...
    email="vamshikrishnamandali@gmail.com"
    password="Vamshi835"
    password = hashlib.md5(password.encode('utf-8')).hexdigest()
    verified=True
    user_type="student"
...
def submitanswer(email=None):
    ...
    if td and not td.testend:
        ...
        # ============================================================ For Auto Display of Status For Single Test ==================================================
        # render_staustable(test_name)
        # IndividualStatus(test_name,email)
...
@app.route('/viewmystatus',methods=['GET'])
@admin_login_required
def viewmystatus():
    # print( "-==-=--=-=-=-==--=-=======-=-=-=======================-=--=-=--===--==--==--=-")
    # print(validate_today())
    result = Tests.query.filter(Tests.start_date<=validate_today(),Tests.end_date>=validate_today()).all()
    # print(result," -------------------------------------------------------------- ")
    return render_template("my.html",tests=result)
...
def readingtask(test_name):
    try:
        app.logger.info("Im in reading task")
        email = get_email_from_session()
        reading_task = None
        test = Tests.query.filter_by(name=test_name).first()
        path = ""
...
def listeningtask(test_name):
    try:
        app.logger.info("Im in listening task")
        email = get_email_from_session()
        listening_task = None
        test = Tests.query.filter_by(name=test_name).first()
        path = ""
...
def get_all_test_created_by(creator=None):
    ...
    for test in result:
        ...
        test.append(button)
        button = "<a href='/delete/"+test[0]+"' class='btn btn-sm btn-danger'>Delete Test</a>"
        test.append(button)
...
@admin_login_required
def deletetest(test_name):
    Test=Tests.query.filter_by(name=test_name).all()
    # mode=Test.test_mode
    # date=Test.startdate
    print(Test," oppoo ");
    result = StudentTests.query.filter_by(test_name=test_name).delete()
    test = Tests.query.filter_by(name=test_name).delete()
    random = Randomize.query.filter_by(test_name=test_name).delete()
    testd = TestDetails.query.filter_by(test_name=test_name).delete()
    essay = EssayTypeResponse.query.filter_by(test_name=test_name).delete()
    content = ContentActivity.query.filter_by(test_name=test_name).delete()
    te = UserAudio.query.filter_by(test_name=test_name).delete()
    db.session.commit()
...
def get_all_student_details(test_name):
    ...
    for student in test_students:
        student1 = get_student_details(student.emailid)
        if student1!=None and student1.email not in student_table:
            student_table[student1.email] = {"name": student1.name, "rollno":student1.rollno}
...
def get_view_details_of_user(email,test_name=None):
    view_details = {}
...
def get_view_details_summary(test_name=None):
...
def get_view_details_complete(test_name=None):
    content_activity = ContentActivity.query.filter_by(test_name=test_name).all()
    view_details = []
    for row in content_activity:
        view = {}
...
def get_responseofIndividualStudent_as_dict(testid=None,email=None):

        table = {}
        result = Response.query.filter_by(test_name=testid,emailid=email).all()
        s=StudentTests.query.filter_by(test_name=testid,emailid=email).all()
        if(len(s)<=0):
            return table
        student = userDetails.query.filter_by(email=email).first()
        # print(student)
        questions = ""
        ...
        for entry in result:
            ...
            # count=1
            # table[rollno].update({
            #                 "Question"+str(count):str(currentQuestion),
...
@app.route('/ViewStatusasCsv/<test_name>')
@admin_login_required
def ViewStatusasCsv(test_name):
    table = get_test_responses_as_dict(test_name)
    data = table.values()
    # print(data)
    return render_csv_status(data,test_name)


@app.route('/ViewStatusasTable/<test_name>')
@admin_login_required
def ViewStatusasTable(test_name):
    # details=individual_students_ques(test_name)
    # return render_template("base.html")
    return render_template("statustable.html",test_name=test_name)


@app.route('/getIndiviualStatusTable/<test_name>/<email>')
@admin_login_required
def getIndiviualStatusTable(test_name,email):
    return render_template('individual_status_table.html',test_name=test_name,email=email)
    # return render_template('individualStatus.html',test_name=test_name,email=email)


@app.route('/getIndiviualStatusCsv/<test_name>/<email>')
@admin_login_required
def getIndiviualStatusCsv(test_name,email):
    tab=get_responseofIndividualStudent_as_dict(test_name,email)
    data=tab.values()
    return render_csv_statusofIndividualStudent(data,test_name,email)

# =============================================================Additional Require Methods============================================================

@app.route('/IndividualStatus/<test_name>/<email>',methods=['GET'])
def IndividualStatus(test_name,email):
    table=get_responseofIndividualStudent_as_dict(test_name,email)
    return json.dumps(table)

@app.route('/getStatusTable/<test_name>',methods=['GET'])
def render_staustable(test_name):
    tables={}
    details=individual_students_ques(test_name)
    students=db.session.query(TestDetails.email).filter(TestDetails.test_name==test_name).order_by(TestDetails.id).all()
    for student in students:
        users = db.session.query(Randomize.user1).filter(Randomize.test_name==test_name,Randomize.user1==student.email).group_by(Randomize.user1,Randomize.test_name).all()
        for user in users:
            items=details[user.user1]
            tables["count"]=len(items)
            tables[user.user1]={}
            count=1;
            co=0;
            for item in items:
                response=Response.query.filter_by(emailid=user.user1,test_name=test_name,currentQuestion=item).all()
                tables[user.user1][count]={}
                if(len(response)!=0):
                    for respon in response:
                        if (respon.submittedans=="skip"):
                            tables[user.user1][count][item]="btn-warning"
                        elif(respon.q_score==1):
                            tables[user.user1][count][item]="btn-success"
                        elif(respon.q_score==0 and len(respon.submittedans)>0):
                            tables[user.user1][count][item]="btn-danger"
                        else:
                            tables[user.user1][count][item]="btn-primary"
                    count+=1
                else:
                    tables[user.user1][count][item]="btn-default"
                    count+=1
                    co+=1
            # print(co,len(items))
            # if (co==len(items)):
...
def render_csv_status(data,test_name):
    csvList=[]
    header = [
                "name",
                "rollno",
    ...
    header.extend(
        [
            "Question_"+"id",
            "Question_"+"_Score",
            "Question_"+"_Submittedans",
            "Question_"+"_Responsetime",
            "Question_"+"_Time",
        ]
    )
    csvList.append(header)
    # use = db.session.query(Randomize.user1).filter(Randomize.test_name==test_name).distinct().all()
    users = db.session.query(Randomize.user1).filter(Randomize.test_name==test_name).group_by(Randomize.user1,Randomize.test_name).all()
    # users=db.session.query(userDetails.email).all()
    # print(users, " bnbnbnbnb")
    # users = StudentTests.query.filter_by(test_name=test_name).all()
    for user in users:
        if user:
            # print(user.email, " bnbnbnbnb")
            Questions_count = Response.query.filter_by(test_name=test_name, emailid=user.user1).count()
            # Questions_count = Response.query.filter_by(test_name=test_name, emailid=user.email).count()
            for csv_line in data:
                #app.logger.info(csv_line)
                # if(csv_line["emailid"]==user.email):
                if (csv_line["emailid"]==user.user1):
                    for i in range(1, Questions_count):
                        ...
                        for lis in csvList:
                            if(lis[2]==user.user1 and lis[3]==csv_line["Question_"+str(i)+""]):
                                csvList.remove(lis)
                                break
                        # if(csv_line["Question_"+str(i)+"_Submittedans"]=='skip'):
                        #     continue                        
                        row.extend(
                                [
    ...
    cw.writerows(csvList)
    output = make_response(si.getvalue())
    output.headers["Content-Disposition"] = "attachment; filename=Status_of_allStudents.csv"
    output.headers["Content-type"] = "text/csv"
    return output

def render_csv_statusofIndividualStudent(data,test_name,email):
    csvList=[]
    header = [
                "name",
                "rollno",
    ...
    csvList.append(header)
    if len(data)>0:
        Questions_count = Response.query.filter_by(test_name=test_name, emailid=email).count()
        # Questions_count = Response.query.filter_by(test_name=test_name, emailid=user.email).count()
        for csv_line in data:
            #app.logger.info(csv_line)
            for i in range(1, Questions_count):
                ...
                for lis in csvList:
                    if(lis[2]==email and lis[3]==csv_line["Question_"+str(i)+""]):
                        csvList.remove(lis)
                        break
                # if(csv_line["Question_"+str(i)+"_Submittedans"]=='skip'):
                #     continue                        
    ...
    cw.writerows(csvList)
    output = make_response(si.getvalue())
    output.headers["Content-Disposition"] = "attachment; filename=Status_of_"+csv_line["name"]+" .csv"
    output.headers["Content-type"] = "text/csv"
    return output


def individual_students_ques(test_id):
    studentsQuestons={}
    students=db.session.query(TestDetails.email).filter(TestDetails.test_name==test_id).order_by(TestDetails.id).all()
    for student in students:
        print(student.email)
        users = db.session.query(Randomize.user1).filter(Randomize.test_name==test_id ,Randomize.user1==student.email).group_by(Randomize.user1,Randomize.test_name).all()
        for user in users:
            arr=[]
            det=Randomize.query.filter_by(test_name=test_id, user1=user.user1).all()
            for x in det:
                arr.append(x.qno)
            studentsQuestons[user.user1]=arr
    return studentsQuestons                                
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if zip_exist or True:
                ...
                # print(file_list)
                # print("===========================================================")
                # print(folder_structure[mode])
                if sublist(folder_structure[mode], file_list):
                    # print("loloolllllllllllllllllllllllllllllllllllllllllllllllllllllllllllll")
                    if mode=="DEP":
                        # print("===============================///////////=======================//////////=====")
...
@app.route('/data')
def register():
    return render_template('profileimg.html')
